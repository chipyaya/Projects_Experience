...
if __name__ == '__main__':
    ...
    # Fit the model
    model.fit(X_train_sample, y_train_sample, epochs=50, batch_size=batch_size)
    ...
    # Get weights and feature vector
    layer_i = 0
    model2 = Sequential()
    if model_type == 'cnn':
        model2.add(Conv1D(filters=filter_num, kernel_size=filter_size, padding='same', activation='relu', input_shape=(X_train.shape[1], X_train.shape[2]), weights=model.layers[layer_i].get_weights(), trainable=False))
        layer_i += 1

        for i in range(conv_layer_num-1):
            model2.add(Conv1D(filters=filter_num, kernel_size=filter_size, padding='same', activation='relu', weights=model.layers[layer_i].get_weights(), trainable=False))
            layer_i += 1
        ...
        if max_over_time_pooling:
            ...
            model2.add(Dropout(dropout_prob1))
            model2.add(Flatten())
            layer_i += 3

        if addOneMoreDense:
            model2.add(Dense(one_more_dense_dim, activation='relu'))
            layer_i += 1

    elif model_type == 'lstm':
        model2.add(LSTM(32, input_shape=(X_train.shape[1], X_train.shape[2]), weights=model.layers[layer_i].get_weights(), trainable=False))
        layer_i += 1

    model2.add(Dropout(dropout_prob2))
    layer_i += 1
    ...
    activation_train = model2.predict(X_train_sample)
    activation_valid = model2.predict(X_valid_sample)
    activation_test = model2.predict(X_test)

    pickle.dump([activation_train, activation_valid, activation_test, weights], open('../activation.p', 'wb'))
    print(activation_train.shape)
    print(activation_valid.shape)
    ...
    print("Accuracy:", scores[1])
    print("Test:")
    scores = model.evaluate(newAct_test, y_test, verbose=0)
    print("Loss:", scores[0])
    print("Accuracy:", scores[1])
